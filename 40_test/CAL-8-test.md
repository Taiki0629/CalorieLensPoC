...
## 6. Phase2-S1: すき家牛丼（2026-07-08・単一画像・GO取得済み）

角度実験用の多角度写真が無く、第三者プロモ写真1枚（moneypost.jp）を使用するため **S1 のみ**
（4モデル×3試行=12件）。画像は著作権により**非コミット**（`.gitignore`）。ログは path+sha256 で参照記録。

```bash
uv run calorielens run --allow-paid --dish sukiya_gyudon --run-id phase2-sukiya-s1  # 12/12 ok
```

正解 **695 kcal**（すき家=ゼンショー公式 栄養成分一覧PDF・更新日2026-07-03）。
※ 検索値795は「高菜明太マヨ牛丼」で誤り→座標付きPDF抽出で素の牛丼並盛=695に訂正（推測禁止の実例）。

| モデル | 推定kcal | APE | (参考:弁当APE) |
|--------|---------:|----:|---------------:|
| kimi-k2.6 | 700〜710 | **1.2%** | 90.7% |
| gpt-5.4 | 720 | 3.6% | 75.6% |
| gpt-5.4-mini | 720 | 3.6% | 69.3% |
| gemma-4-31b-it | 750 | 7.9% | 61.5% |

### 最重要の発見（記事①の主軸を更新）
- **過大評価は「モデルの系統的な癖」ではなく「料理固有」**。定番の牛丼は全モデルほぼ的中（1〜8%）、
  複雑な弁当だけ大きく外す（59〜111%）。→ 単純な「AIは過大評価する」ではなく「**料理の複雑さ・定番度で
  精度が決まる**」がより正確な結論。
- **モデル順位が料理で逆転**: 弁当で最下位の kimi が牛丼で最良（1.2%）、弁当で最良の gemma が牛丼で最下位（7.9%）。
  「万能に最良のモデル」は無い。
- `cost_vs_ape.png` は2料理で下（牛丼・低APE）と上（弁当・高APE）の2集団に明確分離＝「料理 > モデル」を可視化。

> 注: 集約図 `ape_vs_steps.png` は2料理を混ぜる（牛丼はS1のみ・弁当はS1〜S4）ため誤読しやすい。
> **料理別の `ape_vs_steps_{conbini_bento,sukiya_gyudon}.png` と `scores.csv` が正**。

## 7. 残作業
- CAL-8: すき家の**真の角度実験**（S1〜S4）はユーザー自身の4角度撮影が必要（1枚では不可）。
- CAL-4: 弁当のパッケージ写真証跡・商品名/購入店/日、料理名ラベリング（name_accuracy）。
  すき家kcalは公式PDFで確定済み（弁当はユーザー口頭確認のみ＝写真証跡で監査性UP）。
